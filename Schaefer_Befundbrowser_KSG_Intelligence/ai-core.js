(function (root, factory) {
  ...
  function normalizeModel(raw,zdrSet){
    ...
    return {
      ...
      harnessOnly:HARNESS_GATED_IDS.has(String(raw?.id||'')),harnessGated:HARNESS_GATED_IDS.has(String(raw?.id||'')), created:Number(raw?.created||0)||0
    };
  }
  ...
  function isHarnessGatedModel(model){
    const id=String(typeof model==='string'?model:(model?.id||''));
    return HARNESS_GATED_IDS.has(id);
  }
